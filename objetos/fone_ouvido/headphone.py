...
def load_obj_and_texture(objFile, textures_path):
    ...
    for face in modelo['faces']:
        ...
        # Default to blinn3SG if material not recognized
        target = material_data['blinn3SG']
...
def desenha_headphone(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z, material_groups, texture_ids):
    mat_model = model(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z)
    loc_model = glGetUniformLocation(program, "model")
    glUniformMatrix4fv(loc_model, 1, GL_TRUE, mat_model)
    
    # Ordem de renderização
    for material in ['blinn3SG']:
...
while not glfw.window_should_close(window):
    ...
    desenha_headphone(0.0, 0, 1, 0, 0, 0, -10, 1.0, 1.0, 1.0, material_groups, texture_ids)
