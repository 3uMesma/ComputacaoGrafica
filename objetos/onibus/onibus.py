...
def load_obj_and_texture(objFile, textures_path):
    ...
    material_data = {
        'bus': {'verts': [], 'tex_coords': []},
        'base2': {'verts': [], 'tex_coords': []},
        'glass': {'verts': [], 'tex_coords': []}
    }
    ...
    for face in modelo['faces']:
        ...
        if 'bus' in material:
            target = material_data['bus']
        elif 'glass' in material:
            target = material_data['glass']
        else:  # Assume base2 como padrão
            target = material_data['base2']
...
def desenha_onibus(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z, material_groups, texture_ids):
...
while not glfw.window_should_close(window):
    ...
    desenha_onibus(0.0, 0, 1, 0, 0, 0, -10, 1.0, 1.0, 1.0, material_groups, texture_ids)
