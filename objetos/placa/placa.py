...
def load_obj_and_texture(objFile, textures_path):
    ...
    for material, data in material_data.items():
        ...
        material_groups[material] = {
            'start': start,
            'count': count,
            'texture': material_to_texture.get(material, 'Holder_color.png')  # Default
        }
...
material_groups, texture_ids = load_obj_and_texture(
    'uploads_files_2585599_Road+Sign+-+Pare.obj',
    'texturas/'
)


def desenha_placa(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z, material_groups, texture_ids):
...
while not glfw.window_should_close(window):
    ...
    desenha_placa(0.0, 0, 1, 0, 0, -10, -10, 1.0, 1.0, 1.0, material_groups, texture_ids)
