...
def load_obj_and_texture(objFile, textures_path):
    ...
    material_data = {
        'diff': {'verts': [], 'tex_coords': []},
        'phone': {'verts': [], 'tex_coords': []}
    }

    # Classifica cada face no material correto
    for face in modelo['faces']:
        material = face[2].lower()  # Garante minúsculas
        
        verts = circular_sliding_window_of_three(face[0])
        tex_coords = circular_sliding_window_of_three(face[1])
        
        if 'diff' in material:
            target = material_data['diff']
        elif 'phone' in material:
            target = material_data['phone']
        else:
            print(f"Warning: Unknown material '{material}', skipping face")
            continue
...
def desenha_pessoa_telefone(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z, material_groups, texture_ids):
    mat_model = model(angle, r_x, r_y, r_z, t_x, t_y, t_z, s_x, s_y, s_z)
    loc_model = glGetUniformLocation(program, "model")
    glUniformMatrix4fv(loc_model, 1, GL_TRUE, mat_model)
    
    for material in ['diff', 'phone']:
...
while not glfw.window_should_close(window):
    ...
    desenha_pessoa_telefone(0.0, 0, 1, 0, 0, 0, -10, 0.001, 0.001, 0.001, material_groups, texture_ids)
